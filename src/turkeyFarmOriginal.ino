...
void printTestValues(){
  ...
  Serial.println(measureCurrentDC());
  Serial.print(" Potentialmeter value - ");
  Serial.println(measurePotLevel());
  Serial.print(" Reed relay status - ");
...
float measureLevelPressure() {   
  float pressure = bmp.readSealevelPressure(); //Pascals  
  float relativePressure = (pressure/pow(1-(bmp.readAltitude()/44330.0),5.255));    
...
float measureCurrentDC(){
  float I = sensor.getCurrentDC(); //Amperes
  return I;
}
...
int measureLevelLight() {           
  return 1024-analogRead(A1); //Analogue value : 0 - Darkest : 1024 - Brightest
}

//########## Potentialmeter Read #########

int measurePotLevel(){
  return analogRead(A3) - 229;
}
